...
여행 플래너 말고도 코딩 에이전트(탐색→수정→테스트 반복), 딥리서치(읽고 다음 검색을 스스로 결정)가 같은 부류다. 반대로 **프로세스가 명확하고, 지연·비용에 민감하고, 재현·감사가 필요한** 일은 워크플로우가 맞다. 내 예약 안내 서비스가 정확히 여기 속한다. 여행 플래너 말고도 일상에서 주로 접하는 코딩 에이전트(탐색→수정→테스트 반복), 딥리서치(읽고 다음 검색을 스스로 결정하는 과정을 반복)가 같은 부류다.

반대로 **프로세스가 명확하고, 지연과 비용에 민감하고, 재현과 검증이 필요한** 일은 워크플로우가 맞다. 내 예약 안내 서비스가 정확히 여기 속한다.

조금 더 구체적으로, 내 서비스의 특징을 짚어보면 왜 워크플로우로 설계해야만 했는지 분명해진다.

- **질의 유형이 몇 가지로 정해진다** — 예약 안내는 결국 정형 조건 검색 / 의미 검색 / 지도 / 집계 / 일반 안내 정도로 나뉜다. 경로를 미리 그릴 수 있으니 그래프로 고정하는 게 자연스럽다.
- **답이 이미 구축된 데이터 안에 있는 '닫힌' 문제다** — 열린 탐색이 아니라 정해진 범위 내에서 "적절히 찾아 정리"가 핵심이라, 모델이 매 턴 다음 행동을 스스로 궁리할 이유가 없다.
- **실시간 응답이라 지연과 비용이 민감하다** — SSE로 즉답해야 하는데, 매 턴 "다음 뭐 할까?"를 LLM에 되묻는 루프는 그만큼 느리고 비싸다.
- **되돌리기 힘든 행동이 없다** — 예약을 대신 실행하지 않고 안내만 한다. 여행 플래너의 '예약'처럼 위험을 무릅쓴 자율 행동이 필요 없으니, 자율성이 주는 이점도 작다.

이런 서비스에선 자율성보다 **예측 가능성**이 더 중요하다.
